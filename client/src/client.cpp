...
/// One RPC's context, with its deadline and a std::stop_callback that cancels it from whichever
/// thread requests the stop (TryCancel is thread-safe, and cancels a call that has not started
/// yet as soon as it does). Declared after the context, the callback is destroyed first, and its
/// destructor waits for a cancel already running, so the context outlives every use of it.
struct CallContext {
    /// Cancels the call; a named type because a std::stop_callback member needs one.
    struct Cancel {
        grpc::ClientContext * context;  ///< The context of the call to cancel.
        /// Ask gRPC to cancel the call; safe from any thread.
        void operator()() const { context->TryCancel(); }
    };

    /// Apply `options`' deadline and wire its stop token to the context.
    explicit CallContext(const CallOptions & options) : cancel_on_stop(options.stop, Cancel{&context}) {
        init_context(context, options);
    }

    grpc::ClientContext        context;         ///< Passed to the stub for the call.
    std::stop_callback<Cancel> cancel_on_stop;  ///< Must follow context, so it is destroyed first.
};
